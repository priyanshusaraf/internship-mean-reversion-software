"""
ZC M1-M2 Daily Calendar Test — executes zc_calendar_prereg.md EXACTLY.
Phase 1: surrogate-relative VR(q) at N=200 speed gate; if survives, N=500 full + Tier-1 book sim.
Writes data/processed/zc_results.json with full search.

FROZEN PARAMETERS (from zc_calendar_prereg.md):
  DATE_MIN = 1997-10-22, DATE_MAX = 2026-06-04
  SPREAD = ZC1! - ZC2! (β=1 definitional, inner join on UTC date)
...
def build_brn_spread(brn1_path: str, brn2_path: str, date_min: str, date_max: str) -> pd.DataFrame:
    """Load ZC1! and ZC2!, inner-join on UTC date, compute spread = close_1 - close_2."""
    ...
    # Spread = ZC1! close - ZC2! close (β=1 definitional)
...
if __name__ == "__main__":
    ...
    OUT_PATH = os.path.join(root, "data", "processed", "zc_results.json")

    print("=" * 80)
    print("ZC M1-M2 Calendar Test — executing zc_calendar_prereg.md")
    print(f"Frozen seed={BRN_SEED}, jump_k={JUMP_K}, date_min={DATE_MIN}, date_max={DATE_MAX}")
    print("=" * 80)

    # Step 1: Build spread
    print("\n[1] Building β=1 ZC1!−ZC2! spread...")
    brn1_path = os.path.join(DATA_DIR, "CBOT_DL_ZC1!, 1D.csv")
    brn2_path = os.path.join(DATA_DIR, "CBOT_DL_ZC2!, 1D.csv")
    ...
    brn_spread_raw = spread_from_series("ZC_M1M2", spread_df, jump_k=JUMP_K)
    ...
    out = {
        "meta": {
            "instrument": "ZC_M1M2",
            "prereg_doc": "zc_calendar_prereg.md",
            "date_min": DATE_MIN,
            "date_max": DATE_MAX,
